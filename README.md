...
## Description:
A CNN model for the detection of a variety of animal species which are captured with the help of the mobile application. We are making predictions in real-time, as the animal comes in front of the camera, each frame is sent to the system where our model will produce the output within a few microseconds.
...
We collected various datasets of images of various animal species which was present on Kaggle, With the help of the Kaggle api key, we imported these datasets to our Google Colab notebook did some processing, and merged them to make a single dataset then divided the whole dataset into train and test dataset. With the help of the Keras framework, we did image preprocessing on batches of datasets. Then these batches of datasets were applied to our CNN model and based on that network, we made our model learn the features of images and later on classify animal images into classes.
## Data Sources:
